...
tests = testGroup "Cat.Category"
  [ testGroup "(->) instance"
    [ testProperty "left identity: id ∘ f = f" prop_leftIdentity
    , testProperty "right identity: f ∘ id = f" prop_rightIdentity
    , testProperty "associativity: h ∘ (g ∘ f) = (h ∘ g) ∘ f" prop_assoc
    , testProperty "diagrammatic: f ≫ g = g ∘ f" prop_diagrammatic
    ]
