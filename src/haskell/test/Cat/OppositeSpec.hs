...
tests = testGroup "Cat.Opposite"
  [ testGroup "Op (→) instance"
    [ testProperty "left identity: id ∘ f = f" prop_opLeftId
    , testProperty "right identity: f ∘ id = f" prop_opRightId
...
prop_opAssoc (Fun _ h) (Fun _ g) (Fun _ f) x =
  getOp (Op h ∘ (Op g ∘ Op f)) x
    == getOp ((Op h ∘ Op g) ∘ Op f) x

-- Op reverses: composing Op f and Op g gives Op (f ∘ g), not Op (g ∘ f)
prop_opReverse ∷ Fun Int Int → Fun Int Int → Int → Bool
prop_opReverse (Fun _ f) (Fun _ g) x =
  getOp (Op f ∘ Op g) x == (g ∘ f) x
