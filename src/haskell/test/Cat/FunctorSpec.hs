...
tests = testGroup "Cat.Functor"
  [ testGroup "Id functor"
    ...
    , testProperty "composition: cmap (g ∘ f) = cmap g ∘ cmap f" prop_idFunctorComp
    ]
  , testGroup "Maybe functor"
    [ testProperty "identity: cmap id = id" prop_maybeFunctorIdentity
    , testProperty "composition: cmap (g ∘ f) = cmap g ∘ cmap f" prop_maybeFunctorComp
    ]
...
prop_idFunctorComp (Fun _ g) (Fun _ f) x =
  cmap (g ∘ f) (Id x) == (cmap g ∘ cmap f) (Id x)
...
prop_dataFunctorComp (Fun _ g) (Fun _ f) x =
  fmapData idFunctorData (g ∘ f) (Id x)
    == (fmapData idFunctorData g ∘ fmapData idFunctorData f) (Id x)
