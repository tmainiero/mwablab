...
import Cat.NaturalTransformation (NatTrans(..))
...
leftUnitor = NatIso
  { niForward  = NatTrans (\(Paired (x, ())) -> Id x)
  , niBackward = NatTrans (\(Id x) -> Paired (x, ()))
  }
...
prop_idForward mx =
  ...
  in  component (niForward iso) mx == mx

prop_idBackward :: Maybe Int -> Bool
prop_idBackward mx =
  let iso = idNatIso :: NatIso (->) Maybe Maybe
  in  component (niBackward iso) mx == mx

prop_idRoundtrip :: Maybe Int -> Bool
prop_idRoundtrip mx =
  let iso = idNatIso :: NatIso (->) Maybe Maybe
  in  compose (component (niBackward iso)) (component (niForward iso)) mx == mx

-- Left unitor: Paired ~ Id
prop_leftUnitorFwdBwd :: Paired Int -> Bool
prop_leftUnitorFwdBwd px =
  compose (component (niBackward leftUnitor)) (component (niForward leftUnitor)) px == px

prop_leftUnitorBwdFwd :: Id Int -> Bool
prop_leftUnitorBwdFwd x =
  compose (component (niForward leftUnitor)) (component (niBackward leftUnitor)) x == x
...
prop_composeIdLeft px =
  ...
  in  compose (component (niBackward composed)) (component (niForward composed)) px == px
...
prop_invertSwaps x =
  let inv = invertNatIso leftUnitor
  in  component (niForward inv) x == component (niBackward leftUnitor) x
...
prop_doubleInvert px =
  ...
  in  component (niForward inv2) px == component (niForward leftUnitor) px
